...
if(!fs.existsSync(options.input)){
    console.log(`Error input file ${options.input} does not exist`)
    exit();
}

const inputFile = options.input;
const graphvizData = fs.readFileSync(inputFile)

function injectGraphvizIntoTestFile(inputFilePath, outputFilePath, injectableGraphViz){
    var fs = require('fs')
    fs.readFile(inputFilePath, 'utf8', function (err,data) {
        ...
        var result = data.replace(/!!!DIGRAPH!!!/g, injectableGraphViz);
...
injectGraphvizIntoTestFile(
    "./create-anim-file-template.html",
    "create-anim-file.html",
    graphvizData)
